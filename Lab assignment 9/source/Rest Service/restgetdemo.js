...
var url = 'mongodb://<user1>:<123>@ds237815.mlab.com:37815/lab9';
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, callback) {
        db.collection('food').insertOne(response, function(err, result) {
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('food').updateOne({"data.response.venues[i].name"  },{$set:item}, function(err, result) {
            if(err)            {
                res.write(" Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write(" Failed");
            res.end();        }
        db.collection('food').deleteOne({"data.response.venues[i].name"}, function(err, result) {
